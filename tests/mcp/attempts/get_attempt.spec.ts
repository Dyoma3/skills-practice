...
test.group('POST /mcp get_attempt', (group) => {
  ...
  test('gets an owned attempt with its question and rubric', async ({ assert, client }) => {
    const skill = await createSkill(user, 'Capacity estimation')
    const rubric = await RubricFactory.merge({
      type: RubricTypes.Criteria,
      name: 'Capacity scoring',
      description: 'Checks capacity reasoning',
      data: {
    ...
    const question = await QuestionFactory.merge({
      ...
      answer: 'Divide 100 million by 86,400.',
    }).create()
    const attempt = await AttemptFactory.merge({
      questionId: question.id,
      response: 'Approximately 1,157 requests per second.',
      score: 3,
      feedback: 'Correct assumptions and calculation.',
    }).create()

    const response = await callGetAttempt(client, user, { attemptId: attempt.id })
    ...
    assert.deepInclude(result, {
      ...
      question: {
        ...
        context: 'There are 100 million daily requests.',
        answer: 'Divide 100 million by 86,400.',
        createdAt: question.createdAt.toISO(),
        rubric: {
          id: rubric.id,
          type: RubricTypes.Criteria,
          name: 'Capacity scoring',
  ...
  test("rejects another user's attempt", async ({ assert, client }) => {
  ...
  test('requires the mcp:read scope', async ({ assert, client }) => {
    ...
    const response = await callGetAttempt(client, user, { attemptId: attempt.id }, ['mcp:write'])
